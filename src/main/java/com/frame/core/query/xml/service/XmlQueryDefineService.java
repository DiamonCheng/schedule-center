...
public class XmlQueryDefineService {
	...
	public static class DataSetTransferException extends RuntimeException {
		...
		DataSetTransferException(Throwable t) {
			super(t);
		}
	}
	...
	public static class QueryConditionParseException extends RuntimeException {
		...
		QueryConditionParseException(String message) {
			super(message);
		}

		public QueryConditionParseException(Throwable e) {
			super(e);
		}
	}
	...
	public static class ManageExecuteException extends RuntimeException {
		...
		ManageExecuteException(Throwable t) {
			super(t);
		}
	}
	...
	public Object list(PageDefinition pageDefinition, QueryConditions conditions) {
		...
		if (LOGGER.isInfoEnabled())
			...
			System.out.println("生成的查询HQL：" + selectHql);
		...
		for (Map<String, Object> map : listMap) {
			...
			for (Iterator<ColumnDefinition> iterator = pageDefinition.getQueryDefinition().getColumns().iterator(); iterator.hasNext(); j++) {
				...
				if (column.getStaticColumnData() != null)
					stra[j] = column.getStaticColumnData();
					...
					stra[j + 1] = defaultDataFilter.filt(map.get("col_" + j), column);
			}
			list.add(stra);
		}
		return list;
	...
	public int totalPageCount(PageDefinition pageDefinition, QueryConditions conditions) {
		...
		if (LOGGER.isInfoEnabled())
			...
			System.out.println("生成的计数HQL：" + selectHql);
		List<Object> params = new ArrayList<Object>();
		if (conditions.getConditions() != null)
			for (Iterator<QueryCondition> iterator = conditions.getConditions().iterator(); iterator.hasNext();) {
	...
	public QueryConditions prepareQueryCondition(QueryConditions queryConditions, QueryDefinition queryDefinition) {
		...
		for (QueryConditionDefine q : res) {
			...
			if (q.getValue() == null)
				q.setValue(q.getDefaultValue());// 默认查询值
			prepareSelectData(q);
		}
	...
	private void prepareSelectData(QueryConditionDefine q) {
		if ("SELECT".equals(q.getInputType()) || "CHECKBOX".equals(q.getInputType())) {
			if (!StringUtils.isEmpty(q.getStaticData())) {
				q.setParsedData(gson.fromJson(q.getStaticData(), new TypeToken<List<Map<String, String>>>() {
				}.getType()));
			} else {
				if (q.getOptionClass() != null && BaseEntity.class.isAssignableFrom(q.getOptionClass())) {
					...
					throw new QueryConditionParseException("Select option data parse Exception.No static data found nor optionClass found.");
			}
		}
	}

	public void delete(Long id, Class<?> cls) {
		List<?> entity = dao.getHibernateTemplate().find("from  " + cls.getName() + " where id =?", id);
		if(entity.size()>0)
			dao.getHibernateTemplate().delete(entity.get(0));
	}

	/*
	 * public void delete(Class<? extends BaseEntity> cls){
	 * dao.getHibernateTemplate().delete(cls); }
	 */
	@SuppressWarnings("unchecked")
		...
		try {
			...
			for (ManageField manageField : manageFields) {
				...
				if (BaseEntity.class.isAssignableFrom(fieldClass)) {
					...
					// if
					// (manageField.getValue()!=null&&BaseEntity.class.isAssignableFrom(manageField.getValue().getClass()))
	...
	public <T extends BaseEntity> T saveManage(String paramString, GeneralController<T> c) {
			...
			if (fieldValue == null) {
				...
				try {
					...
					for (Object oldEntity : (Collection<?>) fieldValue) {
						newCollectionValue.add(this.<BaseEntity>get((Class<? extends BaseEntity>) manageField.getOptionClass(), (Serializable) ReflectUtil.getValueByField(oldEntity, manageField.getSelectValueField())));
					}
		...
		if (pageHolder.getPageDefinition().getManage().getBeforeManage() != null) {
			...
			try {
				...
				if ((Boolean) toInvoke.invoke(c, args))
					saveOrUpdate(toSave);
				else
					return null;
			} catch (Exception e) {
